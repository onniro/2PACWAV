                                ...
                                int bins_per_point) 
{
    ...
    for(int i = 0; i < num_bins; ++i) 
    {
        ...
        if(norm > 1.0f) 
        { real32_outbuf[i] = 1.0f; }
        else if(norm < 0.0f) 
        { real32_outbuf[i] = 0.0f; }
        else 
        { real32_outbuf[i] = norm; }
    }
}
...
PAC_INTERNAL void test_draw_freq_spectrum(Runtime_Vars *rtvars, Sdl_Apidata *sdldata)
{
...
void main()
{
    ...
    if(!opengl_prep_done) 
    {
        ...
        { 
            platform_dbg_log("vertex shader compiled\n"); 
        }
        ...
        {
            platform_dbg_log("fragment shader compiled\n"); 
        }
