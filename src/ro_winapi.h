...
void *ro_winapi_make_heap_buffer(ro_heap_buffer *buffer, uint64_t bytes) 
{
...
uint64_t ro_buffer_unallocated_bytes(ro_heap_buffer *buffer) 
{
    uint64_t result = 0;
...
void *ro_alloc_buffer_region(struct ro_heap_buffer *buffer, uint64_t region_bytes) 
{
    void *result = 0;
...
void ro_heap_buffer_move_writeptr(ro_heap_buffer *buffer, 
                                ssize_t bytes, 
                                char write_zeroes) 
{
...
char *ro_winapi_get_working_directory(char *destination, DWORD buffer_size) 
{
    char *result = 0;
...
int ro_winapi_file_exists(char *file_path) 
{
    int result = 0;
...
int ro_winapi_directory_exists(char *directory_name) 
{
    int result = 0;
...
int ro_winapi_read_file(char *file_path, char *destination, uint64_t *dest_size) 
{
    int result = false;
...
int ro_winapi_write_file(char *file_path, void *in_buffer, uint64_t buffer_size) 
{
    int result = false;
    HANDLE file_handle = CreateFileA(file_path, 
                            GENERIC_WRITE,
