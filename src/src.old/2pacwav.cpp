...
//i just pasted this stuff idk why it works and the other one doesnt
void pac_nuklearapi_copy_callback(nk_handle handle, const char *data, int data_len)
{
    char *str = 0;
    if(!data_len)
    { return; }
    str = (char*)malloc((size_t)data_len + 1);
    if(!str)
    { return; }
    memcpy(str, data, (size_t)data_len);
    str[data_len] = '\0';
...
#define PAC_NK_NORMAL_BTNCOLOR(...) nk_rgba(0x22, 0x33, 0x33, 0xFF)
#define PAC_NK_HOVER_BTNCOLOR(...) nk_rgba(0x22, 0x44, 0x55, 0xFF)
